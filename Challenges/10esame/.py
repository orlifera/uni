...
def decrypt(input, seed):
    random.seed(seed)
    return reverse_transformation(
        "".join(chr(ord(char) ^ random.randint(80, 120)) for char in input)
    )
...
for seed in range(1000):
    ...
    if (
        set(decrypted).issubset(set(string.ascii_letters + string.digits + "}{_"))
        and "spritz" in decrypted
    ):
        print(decrypted)
        out[seed] = decrypted
